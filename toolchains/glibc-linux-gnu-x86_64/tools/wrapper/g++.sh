...
# 定义工具名称
GCC_NAME="x86_64-buildroot-linux-gnu-g++.br_real"
AR_NAME="x86_64-buildroot-linux-gnu-ar"
LD_NAME="x86_64-buildroot-linux-gnu-ld"

# 1. 环境准备
#    EXECROOT: Bazel 执行时的根目录 (物理路径，不含软链接)
EXECROOT=$(pwd -P)
...
if [[ "${REAL_GCC_ABS}" == "${EXECROOT}"* ]]; then
    ...
    # 极其罕见的情况：编译器不在 execroot 下，只能回退到绝对路径
...
# 检查 LD 是否存在
if [[ ! -f "${REAL_LD}" ]]; then
    echo "ERROR: [g++.sh] Found GCC at ${REAL_GCC_ABS} but LD not found at ${REAL_LD}" >&2
    exit 1
fi
